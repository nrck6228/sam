...
const getDataConfig = () => ({
    ...
    containerId: 'npl-container',
    paginationId: 'npl-pagination',
    itemsPerPage: 12,
    urlPath: '/announcement/npl-npa/page',
    renderTemplate: (item) => `
 ...
 */
const setupDataEngine = async () => {
    const container = document.getElementById('npl-container');
    if (!container) return;

    // ดึงข้อมูลครั้งแรกถ้ายังไม่มี
    if (cachedData.length === 0) {
        const response = await getData();
        cachedData = response?.nplData || [];
    }

    window.nplEngine = initListEngine(getDataConfig());
    const initialPage = window.nplEngine.getInitialPage();
    window.nplEngine.render(initialPage);
};
...
const init = () => {
    ...
    const handleFilter = () => {
        ...
        if (window.nplEngine) {
            window.nplEngine.updateData(newData);
            window.nplEngine.render(1); // กลับไปหน้า 1 เสมอเมื่อ filter
...
window.addEventListener('pageshow', (event) => {
    if (event.persisted) setupDataEngine();
});